...
Stylus is an Arbitrum extension that enables smart contracts to run custom WebAssembly (WASM). Though designed with Rust in mind, Stylus' underlying architecture supports other WASM-compatible languages.

AssemblyScript is a subset of TypeScript that compiles to WASM. With its JavaScript-like syntax and familiar tooling, it can empower a broader range of developers to target Stylus — provided the ABI, memory, and storage expectations are respected.
...
### Assembly Script Stylus (as-stylus)
...
### Assembly Script Interface (as-interface)
...
| Option               | Pros                                      | Cons                        |
|----------------------|-------------------------------------------|------------------------------|
| Rust only            | Full compatibility                        | Poor DX for JS developers    |
| Manual AssemblyScript| Total control                             | Verbose, boilerplate-heavy   |
| SDK + generator ✅    | Best DX + maintainable                     | Requires code generation     |

## Integration with Stylus VM
...
In AssemblyScript, you manually load and store state values by serializing and deserializing data into 32-byte aligned memory blocks. Here’s how a `u64` value can be loaded and stored using the Stylus VM:

- Load a `u64` from storage:
...
Mitigation: Abstract common patterns into utility functions or other language supports and modules to emulate reusability safely.
